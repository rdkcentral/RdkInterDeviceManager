...
#define IDM_DEVICE_DETAILS_INFO		\
    ULONG       helloInterval;		\
    CHAR        ipv4[256];		\
    CHAR        ipv6[1024];		\
    CHAR        capabilities[1024];	\

#define IDM_DEVICE_DETAILS_LIST_INFO	\
    ULONG       status;			\
    CHAR        mac[6];			\
    CHAR        model[256];		\
...
__IDM_DEVICE_LIST
{
    IDM_DEVICE_DETAILS_LIST_INFO
    IDM_DEVICE_DETAILS_INFO
}
IDM_DEVICE_LIST, * PIDM_DEVICE_LIST;

typedef struct
_IDM_
{
    IDM_DEVICE_DETAILS_INFO
    ULONG	detectionWindow; 
    ULONG	numberOfDevices;
    PIDM_DEVICE_LIST	pdeviceList; 
}
IDM_DEVICE_INFO, * PIDM_DEVICE_INFO;

#define BACKEND_MANAGER_CLASS_CONTENT                                       \
        PIDM_DEVICE_INFO  pIDMDeviceInfo;                                   \
