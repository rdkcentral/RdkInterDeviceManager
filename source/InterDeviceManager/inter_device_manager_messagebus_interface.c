...
#ifdef _ANSC_LINUX
DBusHandlerResult
InterDeviceManager_path_message_func
    (
        DBusConnection  *conn,
        DBusMessage     *message,
        void            *user_data
    ...
    )
{
    ...
    {
        CcspTraceError((" !!! InterDeviceManager_MessageBus_Init: component_id or path is NULL !!!\n"));
    }
    ...
    /* Base interface implementation that will only be used by InterDeviceManager */
    cb.initialize             = InterDeviceManager_Mbi_Initialize;
    cb.finalize               = InterDeviceManager_Mbi_Finalize;
    cb.freeResources          = InterDeviceManager_Mbi_FreeResources;
    cb.busCheck               = InterDeviceManager_Mbi_BusCheck;
    ...
    CcspTraceInfo(("CCSP_Message_Bus_Register_Path returnStatus: %d\n", returnStatus));
    ...
    CcspTraceInfo(("CcspBaseIf_Register_Event returnStatus: %d\n", returnStatus));
...
#endif
int
InterDeviceManager_Mbi_Initialize
    (
        void * user_data
    )
{
...
int
InterDeviceManager_Mbi_Finalize
    (
        void*               user_data
    )
{
    ...
    returnStatus = InterDeviceManager_Term();
...
int
InterDeviceManager_Mbi_BusCheck
    (
        void*               user_data
    )
{
...
int
InterDeviceManager_Mbi_FreeResources
    (
        int                 priority,
        void                * user_data
    )
